...
export default function EditListing() {
    ...
    useEffect(() => {
        ...
        const fetchListing = async () => {
            ...
            if(docSnap.exists()){
                ...
                setBedrooms(docSnap.data().bedrooms)
                setDescription(docSnap.data().description)
                setFurnished(docSnap.data().furnished)
                setName(docSnap.data().name)
                setRegularPrice(docSnap.data().regularPrice)
                setOffer(docSnap.data().offer)
                if(docSnap.data().discountedPrice){
                    setDiscountedPrice(docSnap.data().discountedPrice)
                }
                setLoading(false)
            }else{
                navigate("/")
                toast.error("Lisiting could not be found!")
            }
        }
	...
	const onSubmit = async (e) => {
		...
		const docRef = await updateDoc(doc(db, "listings", listingId), formData)
		setLoading(false)
		toast.success("Listing updated successfully!")
		navigate(`/category/${formData.type}/${listingId}`)
	}
